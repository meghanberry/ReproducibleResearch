function STATS_FINAL = xval_lasso_brain(my_outcomes, imgs, varargin)
 ...
 disp('Covariates only: OLS')
        STATS_FINAL.covs = xval_regression_multisubject('ols', my_outcomes, covs);
    end
    ...
    if ~isempty(covs)
        ...
        STATS_FINAL.data = xval_regression_multisubject('lasso', my_outcomes, dat, 'pca', 'ndims', ndims, plsstr);
    else
    ...
    if ~isempty(covs)
     ...
     STATS_FINAL.full_model = xval_regression_multisubject('lasso', my_outcomes, dat, 'pca', 'ndims', ndims, 'cov', covs, plsstr);
    else
        disp('Data only full model: lasso')
             disp('==================================')
   STATS_FINAL.full_model = xval_regression_multisubject('lasso', my_outcomes, dat, 'pca', 'ndims', ndims, plsstr);
    end
    ...
    iimg_reconstruct_vols(m', maskInfo, 'outname', 'xval_lasso_wts_mean.img');
    iimg_reconstruct_vols(m', maskInfo, 'outname', 'xval_lasso_Z.img');

    sig_vox = abs(Z) > bonf_thresh;
    Z_thresh = Z;
    Z_thresh(~sig_vox) = 0;
    iimg_reconstruct_vols(Z_thresh, maskInfo, 'outname', 'xval_lasso_Z_bonf_thresh.img');
    ...
    if dononparam
        ...
        if ~isempty(covs)
            ...
            for i = 1:niter
                ...
                %% Do the prediction
                % ------------------------------------------------------------------
                STATSpcovs = xval_regression_multisubject('lasso', my_outcomesp, covs); %,'pca', 'ndims', ndims);
        ...
        if size(dat{1}, 2) > n_vox
            ...
            for i = 1:niter
                ...
                %% Do the prediction
                % ------------------------------------------------------------------
                STATSpv= xval_regression_multisubject('lasso', my_outcomesp, my_datp, 'pca', 'ndims', ndims);
        ...
        for i = 1:niter
            ...
            %% Do the prediction
            % ------------------------------------------------------------------
            STATSp = xval_regression_multisubject('lasso', my_outcomesp, dat, 'pca', 'ndims', ndims);
