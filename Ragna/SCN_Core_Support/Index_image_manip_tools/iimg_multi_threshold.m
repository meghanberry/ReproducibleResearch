...
%             end
    ...
    if exist('pruneseed', 'var')
        if dotransseed && ~dohideseed
            cluster_orthviews(clseed, {colors(end, :)}, 'overlay', overlay, 'trans', addstr, 'handle', wh_handle);
            addstr = 'add';
        end
    end
    ...
    for i = 1:num_thresholds
        if i == 1 && dohideseed && ~dotransseed
            ...
            cluster_orthviews(cl{i}, {thiscolor}, 'overlay', overlay, 'solid', addstr, 'handle', wh_handle);
