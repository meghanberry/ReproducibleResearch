...
%     overlay = which('scalped_avg152T1_graymatter_smoothed.img');
    ...
    elseif isempty(overlay)
       overlay = which('spm2_single_subj_T1_scalped.img');  % spm2
%         overlay = which('scalped_single_subj_T1.img');  % spm2
    end

    if isempty(overlay)
        
        disp('No valid overlay image specified, and I cannot find the default one.');
        error('The default is spm2_single_subj_T1_scalped.img, and should be on your path.');
    ...
    if dobiv
        ...
        if ~isempty(cols)
            ...
            try Zvals = cat(2,cl{1}.Z); catch error('Invalid clusters.Z field'); end
