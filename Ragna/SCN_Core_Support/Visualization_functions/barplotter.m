...
if lineplot
    for k = 1:max(grouping)
        ...
        hold on
        A(plotcount) = line(1:length(grp),means(grp),'Color',colors{k},'LineStyle',styles{k},'Marker',markers{k});
        count=1;
        for j = grp
            line([count count-0.1 count-0.1; count count+0.1 count+0.1],[err(1, j) err(1, j) err(2, j); err(2, j) err(1, j) err(2, j)],'Color',colors{k},'LineStyle',styles{1});
            count = count + 1;
        end
        hold off
    end
    ...
    for k = 1:max(grouping)
        ...
        for j = grp
            ...
            line(...
                ...
                grouping(j)*0.5+plotcount-0.5 grouping(j)*0.5+plotcount-0.5+0.2 grouping(j)*0.5+plotcount-0.5+0.2], ...
                [err(1, j) err(1, j) err(2, j); err(2, j) err(1, j) err(2, j)], 'Color', 'k');
            hold off
            if lgroups
                if j == grp(end)
