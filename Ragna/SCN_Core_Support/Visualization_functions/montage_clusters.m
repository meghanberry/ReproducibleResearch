...
%     set(mch, 'Position', ssize ./ [0.0303    0.0128    2.0447    1.2577]);
    ...
    if length(cl) > 1
        for i = 2:length(cl)
            ...
            if isempty(colorlimits)
                for j = 1:length(cl{i}),
                    try, if cl{i}(j).numVox == 1, plot_points(mch, cl{i}(j).XYZmm, rc, V, whsl, myc{i}, myplab);end, catch, end
                end
            end
        end
    end
...
function in = scn_mat_conform(in)
    ...
    if in(1) < 0
        ...
        disp('This will be changed to positive for display. Montage_clusters program does not flip images for display.')
