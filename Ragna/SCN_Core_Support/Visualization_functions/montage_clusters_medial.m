...
function mcmh = montage_clusters_medial(ovl,clusters,varargin)
    ...
    if isempty(colorlimits)
        for i = 1:length(clusters),
            try,if clusters(i).numVox == 1, plot_points(mcmh,clusters(i).XYZmm,rc,V,whsl,myc{1},myplab);, end,catch,end
        end
    end
    ...
    if length(cl) > 1
        for i = 2:length(cl)
            ...
            if isempty(colorlimits)
                for j = 1:length(cl{i}),
                    try,if cl{i}(j).numVox == 1, plot_points(mcmh,cl{i}(j).XYZmm,rc,V,whsl,myc{i},myplab);,end,catch,end
                end
            end
        end
    end
