...
function new_files = remove_disdaq_vols(img_files, num_vols_per_run, num_disdaq_vols, varargin)
    ...
    if(ischar(img_files))
        disp('Cellstr not passed in: assuming a list of 4D images')
    ...
    for i=1:length(img_files)
        ...
        if(strict && (num_frames ~= num_vols_per_run + num_disdaq_vols))
            ...
            if(overwriting)
                ...
                [d f e] = fileparts(current_file); %#ok;
